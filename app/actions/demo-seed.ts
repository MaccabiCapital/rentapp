  ...
  const { error: mErr } = await supabase.from('maintenance_requests').insert([
    {
      ...
      notes: `${DEMO_TAG} Reported by tenant via text. Joe confirmed for Friday AM.`,
      photos: [
    ...
    },
    {
      ...
      assigned_to: 'Self',
      notes: `${DEMO_TAG} Ordered replacement latch from Ace, 3-day shipping.`,
      photos: [
    ...
    },
    {
      ...
      description: 'Standing water after cycles. Likely pump or hose issue.',
      urgency: 'normal',
      status: 'open',
      notes: `${DEMO_TAG} Need to pick a vendor — Joe isn't an appliance guy.`,
    },
    {
      owner_id: user.id,
      unit_id: duplexUnit1.id,
      ...
      description: 'Intermittent flicker on the upstairs hallway fixture.',
      urgency: 'low',
      status: 'in_progress',
      assigned_to: 'Maria Sanchez (electrician)',
      notes: `${DEMO_TAG} Maria on-site, replacing ballast.`,
    },
    {
      owner_id: user.id,
      unit_id: duplexUnit1.id,
